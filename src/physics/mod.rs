...
impl PhysicsComp {
    pub fn new() -> Self {
        Self {
            ...
            anglular_velocity: Vector4::zero(),
            can_ever_move: true,
            named: HashMap::new(),
...
pub fn make_octree<'a>(
    arena: &'a Arena,
    values: &[usize],
    phys: &[Option<PhysicsComp>],
    trans: &[Option<TransformComp>],
    bb: BoundingBox,
    depth: usize,
) -> Octree<'a> {
    ...
    for i in values {
        if let Some(id) = phys[*i].as_ref() {
            ...
            let b2 = BoundingBox {
                min: id.min(trans.trans),
                max: id.max(trans.trans),
            };
...
pub fn make_octree_shallow<'a>(
    arena: &'a Arena,
    values: &[usize],
    _phys: &[Option<PhysicsComp>],
    _trans: &[Option<TransformComp>],
    bb: BoundingBox,
) -> Octree<'a> {
    todo!() // Octree::Values { values:values.to_vec() , bx:bb }
}
pub fn check_collision_comps(
    phys_a: &PhysicsComp,
    a_trans: &TransformComp,
    phys_b: &PhysicsComp,
    b_trans: &TransformComp,
    ...
    for i in &phys_a.collisions {
        for j in &phys_b.collisions {
            if let Some(c) = col::check_collision(
                i.col,
                i.offset,
                a_trans.clone(),
                j.col,
...
pub fn update(dt: f64) {
    ...
    let trans_ref = unsafe {
        arena.alloc_array_no_destructor(get_level().transform_comps.list.write().unwrap().as_ref())
    };
    let phys_ref = unsafe {
    ...
    *SAFE_TO_TAKE.lock().unwrap() = true;
    ...
    for i in &iter {
        ...
        let itr: Vec<&usize> = oct
            .query_box(a_phys.bb(a_trans.trans))
            .into_iter()
            .flatten()
            .collect();
        for j in itr {
            ...
            if let Some(c) = check_collision_comps(&a_phys.clone(), &a_trans, b_phys, b_trans) {
                ...
                let (v1, v2) = col::collision_response(
                    p_i.mass(),
                    p_i.velocity,
                    p_j.mass(),
                    p_j.velocity,
...
pub fn create_box(pos: Vector3, vel: Vector3, tint: Color) -> Entity {
    ...
    let sz = 0.05;
    cmp.velocity = vel;
    cmp.collisions.push(Collision {
        col: BoundingBox {
            min: Vector3::new(-sz, -sz, -sz),
            max: Vector3::new(sz, sz, sz),
        },
