...
impl ArenaInternal {
    ...
    pub fn new_sized(size: usize) -> ArenaInternal {
        unsafe {
            ...
            Self {
                ...
                previous_allocation: ptr,
                next: None,
                defer_que: std::ptr::null_mut(),
            }
        }
    }
    pub unsafe fn alloc_bytes(&mut self, count: usize) -> &mut [u8] {
        unsafe {
            let fia = if count % 16 == 0 {
                count
            } else {
                count + (16 - count % 16)
            };
    ...
    pub unsafe fn alloc_no_drop<T>(&mut self, value: T) -> &mut T {
        let out = unsafe {
            let ptr = self.alloc_bytes(size_of::<T>()).as_ptr() as *mut MaybeUninit<T>;
            assert!(!ptr.is_null());
    ...
    pub fn defer_fn(&mut self, func: fn(*mut c_void), data: *mut c_void) {
        let mut def = ArenaInternalDefer {
            func,
            data,
...
impl Arena {
    pub fn new() -> Box<Self> {
        Box::new(Self {
            internal: UnsafeCell::new(ArenaInternal::new()),
            destructors: Mutex::new(HashSet::new()),
        })
    }
    pub fn new_sized(size: usize) -> Box<Self> {
        Box::new(Self {
            internal: UnsafeCell::new(ArenaInternal::new_sized(size)),
            destructors: Mutex::new(HashSet::new()),
        })
    }
    ...
    fn clone(&self) -> Self {
        unsafe {
            let new_items: &mut [T] = self
                .arena
                .alloc_array_space::<T>(self.capacity)
                .assume_init();
        ...
        if self.length < self.capacity {
            ...
            unsafe {
                ...
                if new_items.as_ptr() != self.items {
                    for i in 0..self.length {
                        let _ = ManuallyDrop::new(std::mem::replace(
                            &mut new_items[i],
                            self.get(i).clone(),
                        ));
                    }
                ...
                self.length += 1;
                let _: ManuallyDrop<T> =
                    ManuallyDrop::new(std::mem::replace(self.get_mut(self.length - 1), v));
            }
        }
    }
...
impl<'a, T> GraphNode<'a, T> {
    pub fn new(arena: &'a Arena, value: T) -> &'a mut Self {
        let base = RefCell::new(GraphInternal {
            value,
            edges: AVec::new(arena),
            reached: false,
        });
